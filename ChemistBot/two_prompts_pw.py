# incomplete, 2 prompts need to be separated
import os
import json
import fitz  # PyMuPDF
import pandas as pd
...
FILE_INPUT_PATH = r"C:\Users\User1\Downloads\pymu_error\orgfile\Sri Lakshmi Sai MA_June_25_PWS_spd pw.pdf"
JSON_OUTPUT_PATH = r"C:\Users\User1\Downloads\Sri Lakshmi Sai MA_June_25_PWS_spd pw.json"
...
def extract_input_text(input_path: str) -> str:
    ...
    if ext == ".pdf":
        return extract_pdf_text_fitz(input_path)
...
- When product name is written before customer name, handle those files properly without writing "ICode" as product name.
...
- `"sales"` → Quantity sold, Never mix with "MRP", "Rate" or "Area"
...
def detect_layout(pdf_text: str) -> str:
    lines = [l.strip() for l in pdf_text.splitlines() if l.strip()]

    product_like_lines = 0
    customer_like_lines = 0

    for i, line in enumerate(lines[:200]):
        if line.isupper() and len(line.split()) <= 4:
            if i + 1 < len(lines) and any(
                kw in lines[i + 1].upper()
                for kw in ["MEDICAL", "PHARMA", "MEDIC", "MEDICALS"]
            ):
                product_like_lines += 1

        if any(kw in line.upper() for kw in ["MEDICAL", "PHARMA", "MEDICALS"]):
            customer_like_lines += 1

    if product_like_lines >= 2:
