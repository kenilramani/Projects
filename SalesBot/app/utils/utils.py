...
from app.core.state import BotState, BotRequest
...
def convert_to_botstate(fastapi_request: BotRequest) -> BotState:
    ...
    """
    try:
        from app.database.session_manager import get_or_create_session # Local import to avoid circular dependency if any
...
def get_individual_token_usage(state, result,latest_agent)->dict:
    try:
        # Merge all raw responses (main agent + additional ones from guardrails/tools)
        # We tag main agent responses first so get_consumption_info knows what they are
        for i, resp in enumerate(result.raw_responses):
            ...
            # Always tag the primary model name so consumption_info shows the real model string
            # rather than a router alias (e.g. "primary") that response.model might contain.
            if not getattr(resp, "_actual_model_name", None):
                setattr(resp, "_actual_model_name", settings.primary_model)
        all_raw_responses = []
