...
Usage:
    ...
    from app.callbacks.handlers import on_sales_handoff
...
from app.core.models import BotState, HandoffArgs
...
@track
def on_demo_handoff(ctx: RunContextWrapper[BotState], Args: HandoffArgs) -> BotState:
    """
    Callback function when handing off to demo booking agent.
    ...
    """
    logger.info("*" * 60)
    logger.info("[on_demo_handoff] Demo Booking Handoff Initiated")
    ...
    try:
        ...
        logger.info("[on_demo_handoff] State updated with new_booking=True")
        logger.info("*" * 60)

        return state

    except Exception as e:
        logger.error(f"Error in demo handoff: {e}")
        logger.exception("Full traceback:")
        return ctx.context
