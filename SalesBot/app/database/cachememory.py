...
AZURE_EMBED_ENDPOINT = Settings().AZURE_EMBED_ENDPOINT
AZURE_INFERENCE_KEY = Settings().azure_inference_credential
AZURE_EMBED_DEPLOYMENT = Settings().embedding_model
...
def retrieve_from_cache(
    ...
    for pair in session["qa_pairs"]:
        cached_question = pair["user"]

        cached_embedding = _embed(cached_question).reshape(1, -1)
