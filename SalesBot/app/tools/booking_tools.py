...
def process_booking_datetime(
    ...
    try:
        ...
        # Resolve working_hours early so it's available for both the
        # time_defaulted pre-validation block AND the full Step 2 validation.
        working_hours = None
        try:
            last_agent = ""
            if (
                ctx
                and ctx.context
                and ctx.context.user_context
                and ctx.context.user_context.last_agent
            ):
                last_agent = str(ctx.context.user_context.last_agent).lower()
            if "demo" in last_agent or "booking" in last_agent:
            ...
            logger.debug(f"Could not determine agent for persona working hours: {e}")
        ...
        if parsed.get("time_defaulted", False):
            ...
            pre_validation = _validate_booking_datetime(
                date_str_pre, "10:00", timezone, working_hours
            )
            if (
        ...
        validation = _validate_booking_datetime(
            date_str, time_str, timezone, working_hours
        )
