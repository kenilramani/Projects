...
def sales_engine_prompt(state: BotState) -> str:
  ...
  If declined → Mention 1-2 alternative products
    ...
    return f"""<role>
You are {name}, {personality_context}, {industry_context} consultant from {company_name}.
...
- Contact details not explicitly present in persona (phone numbers, emails beyond what is in <resources>)
...
<resources fallback_only="true" note="Always check knowledge base first">
<company>
Name: {state.bot_persona.company_name}
...
- Stay under 30 words. DO NOT exceed word limits.
</structure>
</style_rules>
...
<quality_check>
Before finalizing:
- Re-read response and rephrase if robotic, repetitive, or templated
...
{{
  "response": "<objection_handle_agent response as-is>",
  "probing_details": {{
