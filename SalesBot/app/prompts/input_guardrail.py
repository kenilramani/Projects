""""
This File has all PROMPTS used by Guardrail agents in the system.
"""

from app.core.state import BotState
from app.utils.utils import convert_to_toon, format_chat_history
...
def input_guardrail_prompt(state: BotState) -> str:
    ...
    if persona is None:
        return """You are a security classifier for a business chatbot.
...
"""
    ...
    chat_history = state.user_context.chat_history[-3:]  # Limit to last 3 messages for context
    chat_history = convert_to_toon(chat_history)

    user_query = convert_to_toon(state.user_context.user_query)
    chatbot_name = persona.name
    ...
    industry_context = convert_to_toon(persona.business_focus if persona.business_focus else "general business")
    goal_context = convert_to_toon(persona.goal_type if persona.goal_type else "customer engagement")
    product_names = ([convert_to_toon(p.name) for p in persona.company_products] if persona.company_products else [])
    product_list = ", ".join(product_names) if product_names else "our products and services"
    features_context = convert_to_toon(persona.core_features if persona.core_features else "our features")
    usps_context = convert_to_toon(persona.core_usps if persona.core_usps else "our value propositions")
    personality_context = convert_to_toon(persona.personality if persona.personality else "professional and helpful")
    current_cta = convert_to_toon(persona.current_cta if persona.current_cta else "book a demo")
    category = convert_to_toon(persona.category if getattr(persona, "category", None) else None)
    sub_category = convert_to_toon(persona.sub_category if getattr(persona, "sub_category", None) else None)
    business_type = convert_to_toon(persona.business_type if getattr(persona, "business_type", None) else None)
    offer_description = convert_to_toon(persona.offer_description if getattr(persona, "offer_description", None) else None)
    contact_info = persona.contact_info if getattr(persona, "contact_info", None) else None
    working_hours = state.bot_persona.working_hours if getattr(state.bot_persona, "working_hours", None) else "our working hours"
# Build available assets block from persona
    ...
    if contact_info:
        if isinstance(contact_info, dict):
            contact_info_str = ", ".join(f"{k}: {convert_to_toon(v)}" for k, v in contact_info.items() if v)
    ...
    return f"""You are {chatbot_name} from {company_name} ({industry_context}).
...
Business requests (booking, demos, pricing, scheduling, product info, brochures, assets, objections, support, general questions) are ALWAYS safe.
...
NEVER attacks: booking, demo requests, pricing, scheduling, product questions, brochures, assets, objections, support, company info, off-topic chat, unrecognized text.
...
16. Code or script requests — generating, fixing, or explaining programming code or scripts e.g., "Write Python code", "Debug my JavaScript", "Give me a bash script", "Write a program"
...
- Speak as {chatbot_name}, warm and conversational. Under 40 words.
