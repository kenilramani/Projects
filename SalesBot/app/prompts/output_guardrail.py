...
def output_guardrail_prompt(state: BotState) -> str:
    ...
    # Build product list
    product_names = (
        [convert_to_toon(p.name) for p in state.bot_persona.company_products]
...
2. INFORMATION ACCURACY (CRITICAL)
   ...
   PRICING: Has pricing → extract; No pricing → "I'd be happy to walk you through pricing during the demo"; NEVER guess
...
3. CONTACT INFO (CRITICAL)
   ...
   REQUIRED: Check {contact_info}; exists → share naturally; not exists → suggest demo
...
4. TONE & PERSONALITY (HIGH)
   ...
   LIMITS: main <45 words, demo/objections <70 words
...
8. BOOKING FLOW (CRITICAL - demo only)
