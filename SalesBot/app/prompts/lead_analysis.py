...
def lead_analysis_prompt(context, agent):
    ...
    return f"""You are an expert sales analyst specializing in lead qualification. Based on the conversation or lead details provided, classify the lead into one of three categories - hot, warm, or cold- according to their eagerness and urgency to book a demo.

### OBJECTIVE
Classify the lead into one of the following categories based solely on their demo eagerness:
1. Carefully analyze the provided conversation or lead details.  
2. Classify the lead strictly based on how quickly and eagerly they move toward booking a demo.  
...
### DEMO-EAGERNESS CLASSIFICATION GUIDE

- Classify as hot IF Shows clear, immediate intent to book a demo.  
    - Instantly agrees to schedule a demo (e.g., “let's do it now,” “in 5 mins,” “today”).
    - Quickly provides all required contact details (name, email, phone, product, date & time) without hesitation.
    - Actively asks for confirmation or demo link.
    - Specifies a clear demo time/date and has minimal objections.
...
- Classify as warm IF Shows interest but not urgency or clarity in scheduling. 
    ...
    - Requests to reschedule or changes the demo timing at least once  
    - Seeks more information before confirming.  
    - Gives partial information or tentative answers.  

- Classify as cold IF Shows low or no real intent or disengaged to book a demo.    
    - Avoids or delays demo discussion entirely.  
    - Changes the topic during or after demo confirmation talk.  
    - Does not specify any demo timing, date, or day.
...
- Analyze demo eagerness on a scale of 0-1 (0 = no interest, 1 = extremely eager)
...
{{
    "lead_analysis": {{
        ...
        "recommended_next_action": "Specific action to take (e.g., 'Send demo link immediately', 'Schedule follow-up call next week', 'Add to nurture sequence')",
    }}
}}
