...
Usage:
    from app.core.guardrail import input_attack, output_guardrail
...
from app.prompts import input_guardrail_prompt, output_guardrail_prompt
...
# Create the input guardrail agent
        ...
        if hasattr(ctx, "context") and ctx.context is not None:
            ...
            if hasattr(state, "additional_raw_responses"):
                for resp in result.raw_responses:
                    ...
                    # Tag the actual model name so consumption info shows the correct model
                    # (response.model may hold the router alias or primary model name)
                    setattr(resp, "_actual_model_name", Settings().guardrail_model)
...
output_guardrail_agent = Agent(
    ...
    model=RouterModel(model="guardrail"),
    model_settings=_model_settings,
...
async def output_guardrail(
    ...
    try:
        ...
        logger.info(f"[output_guardrail] Tripwire triggered: {validation_failed}")
        logger.info("-" * 50)
