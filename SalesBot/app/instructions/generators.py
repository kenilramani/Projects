"""
Instruction Generators - Dynamic instruction generation for agents.
...
Usage:
    from app.instructions.generators import InstructionBuilder
...
from app.core.models import BotState, BotPersona, UserContext
...
class InstructionBuilder:
    ...
    to generate context-aware prompts for different agents.
    ...
    def build_sales_instructions(self) -> str:
        ...
        """
        from app.prompts import sales_prompt

        return sales_prompt(self.state)

    def build_demo_instructions(self) -> str:
        """
        Build instructions for the demo booking agent.

        Returns:
            Formatted demo booking prompt
        """
        from app.prompts import demo_prompt

        return demo_prompt(self.state)
    ...
    def build_proceed_email_instructions(self) -> str:
        """
        Build instructions for the proceed with email agent.

        Returns:
            Formatted proceed email prompt
        """
        from app.prompts import proceed_with_email_prompt

        return proceed_with_email_prompt(self.state)
...
def build_demo_instructions(state: BotState) -> str:
    """
    Build instructions for the demo booking agent.
    ...
    Returns:
        Formatted demo booking prompt
    """
    return InstructionBuilder(state).build_demo_instructions()
