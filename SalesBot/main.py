...
from app.database.session_manager import init_memory_db
from app_agent import run_chatbot_api
from app.utils.utils import convert_to_botstate
...
async def chat_endpoint(request: BotRequest) -> APIResponse:
    ...
    try:
        ...
        # Run the agent
        updated_state = await run_chatbot_api(state)
...
async def generate_executive_summary_endpoint(request: ExecutiveSummaryRequest = Body(...)):
    """
    Standalone endpoint to generate an executive summary on demand.
    Uses agent_result as the primary input; falls back to chat_history if empty.
    No conditions — generates a summary every time it is called.
    """
    from app.database.executive_summary import generate_executive_summary
...
async def activity_summary(request: AllSummary) -> ActivitySummary:
    ...
    """
    from app.agents.activity_summary import run_activity_summary_agent
    logger.info("=" * 100)
...
async def generate_probing_questions(request: ProbingRequest = Body(...)):
    """
    Endpoint to generate probing questions using the probing agent.
    """
    from app.agents.probing import run_probing_agent
    ...
    try:
        ...
        return result  # should be List[ProbingQuestion]
...
async def chat_streamlit(request: BotRequest) -> dict:
    ...
    from app.utils.utils import convert_to_botstate, model_to_dict
...
async def autofill_persona(request: AutofillPersonaRequest):
    """
    ...
    """
    from app.database.session_manager import get_or_create_session
    ...
    try:
        ...
        from app.agents.crawl_persona import run_crawl_persona_agent
...
async def generate_probing_instructions_endpoint(
    ...
    try:
        # Lazy import to speed up server startup
        from app.agents.probing_instruction import generate_probing_question_instructions
...
async def generate_templates_endpoint(request: TemplateGenerationRequest = Body(...)):
    """
    Generate Templates for each product.
    """
    custom_persona = request.custom_persona or {}
    ...
    try:
        # Lazy import to speed up server startup
        from app.agents.template_generation import run_template_generation_agent
