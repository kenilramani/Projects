...
This module provides the core chatbot functionality including:
...
Usage:
    from app_agent import run_chatbot_api, create_bot_state
    
    state = create_bot_state(user_id="123", user_query="Hello")
    result_state = await run_chatbot_api(state)
...
from app.config import settings, MAX_HISTORY
...
from app.core.probing_state import ProbingEngineState
from app.core.negotiation import NegotiationEngine

# Import agent factory
from app.agents.factory import root_agent

# Database imports (not modified per requirements)
from app.database.session_manager import init_memory_db, save_state

from app.database.cachememory import (
    get_session,
    init_session,
    update_session,
    retrieve_from_cache,
)
from app.utils.prompt_cache import cache_monitor
from app.utils.utils import get_individual_token_usage
from app.database.agent_session import get_agent_session, dispose_engine
...
async def finalize_bot_state(state: BotState, result: Any, user_query: str) -> BotState:
    ...
    if is_empty_response:
        ...
        # Check if we're in a demo booking flow
        last_agent = state.user_context.last_agent or ""
        company_name = (
            state.bot_persona.company_name if state.bot_persona else "our company"
        )

        if "demo" in last_agent.lower() or "booking" in last_agent.lower():
            # In demo booking flow - ask for specific details
            fallback_response = "To help you book a demo, could you let me know which product you're interested in and what date and time works best for you? 📅"
    ...
    # _update_negotiation_dynamic_state(state, result)
...
@track
async def run_chatbot_api(state: BotState) -> BotState:
    """
    ...
    logger.info("=" * 100)
    logger.info("[run_chatbot_api] Starting chatbot execution")
...
#     # Create a per-user SQLAlchemy session so each user's history is isolated
#     user_id = state.user_context.user_id
#     logger.info(f"Creating SQLAlchemySession for user: {user_id}")
#     current_session = await get_agent_session(user_id=user_id)
 
#     try:
#         context = RunContextWrapper(state)
 
#         # Extract the plain user-query string from the input list.
#         # The SQLAlchemySession framework automatically merges the query with
#         # the stored conversation history, so we pass a string here instead
#         # of a pre-built message list.
#         user_query = (
#             agent_input[0].get("content", "")
#             if agent_input and isinstance(agent_input, list)
#             else str(agent_input)
#         )
 
#         run_config = RunConfig()
 
#         result = await Runner.run(
#             starting_agent=root_agent(),
#             input=user_query,
...
#         logger.info("|" * 60)
#         logger.info(f"Result from Runner: {result}")
#         logger.info("|" * 60)
...
async def _execute_agent(state: BotState, agent_input: List[Dict[str, str]]) -> Any:
    ...
    try:
        ...
        result = await Runner.run(
            starting_agent=root_agent(), input=agent_input, context=context.context
        )
        ...
        if (
            ...
            class FallbackResult:
                """Fallback result for model behavior errors."""
...
def _handle_guardrail_tripwire(error: OutputGuardrailTripwireTriggered) -> Any:
    ...
    if error.args:
        ...
        if isinstance(guardrail_result, str):
            ...
            # Try to access the result from the module state
            from app.core.guardrail import get_output_guardrail_decision

            output_decision = get_output_guardrail_decision()
            if output_decision:
                logger.info(f"Got output decision from module state: {output_decision}")
                if (
                    hasattr(output_decision, "suggested_text")
                    and output_decision.suggested_text
                ):
                    suggested_text = output_decision.suggested_text
        else:
    ...
    if not suggested_text:
        ...
        # Try to get from module state one more time
        try:
            from app.core.guardrail import get_output_guardrail_decision
            ...
            if (
                output_decision
                and hasattr(output_decision, "suggested_text")
                and output_decision.suggested_text
            ):
                suggested_text = output_decision.suggested_text
                logger.info(f"Found suggested text from module state: {suggested_text}")
        except Exception as e:
            logger.warning(f"Could not get guardrail decision from module state: {e}")
    ...
    # Create a result object with the suggested text
...
async def _handle_input_guardrail_tripwire(
    ...
    guardrail_response = ""
    try:
        from app.core.guardrail import get_input_guardrail_decision

        input_decision = get_input_guardrail_decision()
        if input_decision:
            logger.info(f"Input guardrail decision: {input_decision}")
            if hasattr(state, "input_guardrail_decision"):
                state.input_guardrail_decision = input_decision
            # Extract the pre-generated response from the guardrail output
            guardrail_response = getattr(input_decision, "response", "") or ""
    except Exception as e:
        logger.warning(f"Could not record input guardrail decision: {e}")
